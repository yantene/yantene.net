# 0004. コンテンツリポジトリを GitHub リポジトリに置き、D1 / R2 をキャッシュにする

- Status: Deprecated ([0034](0034-artifacts-as-content-source-of-truth.md) で置き換え)
- Date: 2026-07-05
- Deciders: @yantene

## Context / 背景

ノート (Markdown + 画像) のコンテンツリポジトリをどこに置き、どう配信するかを決める必要がある。
要件は以下の通り。
...
- バージョン管理があること
- 配信 (一覧・詳細・画像) がコンテンツリポジトリのレイテンシやレート制限に引きずられないこと

## 検討した選択肢

- **案 A: R2 をコンテンツリポジトリにする** — rclone 等で R2 に直接 sync する。
...
- **案 C: GitHub リポジトリをコンテンツリポジトリにし、D1 / R2 をキャッシュにする (採用)** —
  リクエストは D1 + R2 だけで捌き、コンテンツリポジトリには同期 (refresh) のときだけ触る。
  - Pros: `git push` で完結する。バージョン管理は git がそのまま与える。パース規則は
    アプリ内の 1 箇所にある。配信経路がコンテンツリポジトリから切り離される。
  - Cons: コンテンツリポジトリが Cloudflare 外にある。3 層 (GitHub / D1 / R2) の構成が複雑。

## 決定

案 C を採用する。

- コンテンツリポジトリは GitHub リポジトリ (`yantene/notes`)。`notes/<slug>.md` が本文、
  `notes/<slug>/<filename>` が画像アセット。
- D1 はメタデータインデックス (スラグ・タイトル・要約・公開日・更新日・タグ・
  コンテンツハッシュ・閲覧数) を持つ。
- R2 は原文 Markdown・パース済み MDAST・画像をキャッシュする。
- **通常のリクエストでは D1 + R2 からのみ配信し、コンテンツリポジトリには触らない。**
- コンテンツリポジトリ → D1 / R2 の同期は `POST /api/v1/refresh` のときだけ行う。ユーザー認証ではなく
...
- 変更検出はコンテンツハッシュで行う。refresh 時に git tree API
  ...
  変わったノートだけ内容を読み直して再パース・upsert する。コンテンツリポジトリから消えたノートは
  D1 / R2 から掃除する。
...
- 良い面: `git push` だけでコンテンツが反映される自然なワークフロー。バージョン管理が
  git からそのまま得られる。配信経路が R2 + D1 に閉じるので、コンテンツリポジトリの障害やレート制限が
  読者に届かない。
- 悪い面・トレードオフ: コンテンツリポジトリが Cloudflare 外 (GitHub) にある。3 層構成の複雑さ。
...
- 検証方法: `github-content-store.test.ts` が fetch モックでツリー取得・ファイル読み取り・
  ...
  削除時の掃除・force の挙動を固定する。通常リクエストがコンテンツリポジトリに触らないことは、
...
(コンテンツリポジトリを GitHub に置く / D1 と R2 は写し) は変えていない。

追記ではなく本文の訂正で処理したのは、ここが**次に実装へ触る人が前提として読む場所**
だからである。誤った記述を残したまま下に訂正を足すと、上だけを読んだ人が
「base64 を復号している」という前提で改修に入る。[ADR 0007](0007-strict-csp-outside-development.md) /
[ADR 0013](0013-math-as-mathml-at-refresh-time.md) が防御の範囲について同じ判断をしているのに倣った。
