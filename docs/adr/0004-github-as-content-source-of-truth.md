...
- infra の `GitHubContentStore` が実装する。ツリーは git tree API、ファイル内容は
  contents API で取得し、base64 を復号して生バイト列を返す。Workers の fetch キャッシュが
  push 直後の古いツリーを返さないよう `cache: "no-store"` で読む。
