...
リンクには従来どおり入らない (rehype-sanitize の既定が落とす)。本文の生 HTML は iframe 以外が
落ちるので、書き手が MathML を直に書いて `style` を差し込むこともできない。

つまり inline style が通る経路は「refresh 時に Temml が組んだ MathML」だけである。
