...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
  echo "Usage: $0 <bucket_name> [--local|--remote]"
  exit 1
fi

BUCKET_NAME="$1"
STORAGE_FLAG="${2:---local}"

if [ "$STORAGE_FLAG" != "--local" ] && [ "$STORAGE_FLAG" != "--remote" ]; then
...
echo "Done: ${file_count} file(s) uploaded to '${BUCKET_NAME}' (${STORAGE_FLAG})."
