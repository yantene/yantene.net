...
`/about` に書き手のプロフィールを出す。中身はコンテンツリポジトリの `profile.md` 1 つで、
記事と同じ refresh が D1 と R2 に同期する
([ADR 0041](../../docs/adr/0041-sync-the-profile-as-one-content-file.md))。

- フロントマターが構造を持つ (名前・生年月日・出身地・短い自己紹介・出ていく先)
- **経歴と好きなものは本文に書く。** 構造では持たない。`## History` の箇条書きがそのまま
  出る。年別のアーカイブに学年を注釈する構想 (#414) が要る構造は、そのときに決め直す
- 保存先は D1 の 1 行 (`id = 1` に固定) と R2 の `profile/mdast.json`
- **書式は `content/AGENTS.md`。** 欄を足すときは両方を直すことになる

読めないフロントマターはファイルごとスキップして理由を返す (記事と同じ fail-loud)。
知らない `platform` を書いたときも同じで、絵の無いリンクを黙って落とさない。絵の表は
`components/social/social-links.tsx` にあり、そこに足すのが先になる。

### 中身が無いうちは「準備中」に倒す

初回の refresh の前と、`profile.md` を消した直後は D1 に行が無い。**そこで 500 を出さない。**
ナビは常に About を指しているので、行き先が落ちるとサイトが壊れて見える。

`noindex` を立てるのも、sitemap から外すのも、このときだけ。**3 つは必ず一緒に動かす。**

| 何             | どこ                                  |
| -------------- | ------------------------------------- |
| `noindex`      | `app/frontend/routes/about.tsx`       |
| sitemap        | `app/backend/handlers/seo.handler.ts` |
| スモークの期待 | `scripts/smoke.mjs`                   |

### 顔と、代表 h-card

顔はやんてねくんのアイコン (`/icons/icon-512.png`)。近影を出すのと、
`/api/v1/profile/assets/<path>` を生やすのは #413 の残り。

**代表 h-card はトップのヒーローのまま。** `/about` にも `h-card` を置くが、同じ人を
指す別の h-card になる。代表が 2 つあるとどちらを読むかが決まらないので、`/about` の
`u-url` はページ自身ではなく出ていく先に付けてある。

トップのヒーローの 3 行と、記事の末尾の筆者紹介をプロフィール由来にするのは #413 の残り。
いまはヒーローの 3 行がコードに直書きされたままで、`profile.md` の tagline とは繋がって
いない (**書き換えると 2 か所が食い違う**)。
