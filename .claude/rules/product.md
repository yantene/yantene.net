...
書き手が何者かは**コンテンツリポジトリの `profile.md` + `profile/`** にある (顔を除く。下記)。
記事と同じ対の形で、
...
- **生い立ちの欄は持たない。** 生年月日や出身地を名乗りに並べると表になり、顔・名前・
  短い自己紹介という主役が押される。そういう話は本文の地の文で書く (#508)。
  機械が読む形 (`dt-bday` や JSON-LD の `birthDate`) も持たない
- 長い自己紹介に貼った画像は `/api/v1/profile/assets/<path>` で出す。コンテンツリポジトリの
  URL は露出させない (記事のアセット API と同じ)
...
- **経歴の年表は `/about` の末尾に出す。束ねるのは暦年ではなく章**
  ([ADR 0044](../../docs/adr/0044-show-the-history-as-a-timeline-of-chapters.md))。
  一度「持たない」と決めて覆した。当時の懸念は「学歴や資格を並べると、読み手が最初に
  受け取るのが履歴書になる」ことで、**それを押さえているのは置き場所と束ね方である**。

  ⚠️ **上へ動かすと懸念がそのまま戻る。** 出すのは長い自己紹介の本文と作ったものより
  後で、名乗り (`ProfileCard`) にも本文の頭にも置かない (`about.placement.test.tsx` が
  節の並びを固定している)。束ねるのは章 (高校・大学・
  社会人) で、これは書き手が決める区切り。暦年で束ねると、2012 年に 4 件寄る一方で
  2013 〜 2015 年が空き、卒業と入学が同じ束に混ざる。

  **日は 3 段の精度 (`2011` / `2011-06` / `2011-06-04`) で、終わりのある出来事には
  `until` を添える。** 揃えるために覚えていない日を捏造させない。札は重なる位を畳んで
  出す (`2012-08-14 〜 18`)。**日より細かくは持たない**し、片側だけ開いた範囲も持たない
  (続いていることは始まった日の出来事として書く)。

  **並べ直さない。** 日付が入っている以上できるが、**章は書き手の語り**なので順も
  書き手が決める。

  **トップと記事の末尾は読まない。** 経歴は `Profile` 集約が持たず、`findHistory` という
  別の読み口から引く。**書き込みだけは一緒** (`upsert(profile, history)` が 1 つの batch に
  収める。割ると「名前はあるのに経歴が空」が表に出る)。

  ⚠️ **口が分かれているので、倒れ方も分かれる。** 保存されている行が読めなくなったとき、
  プロフィールが読めなければ `/about` はページごと「準備中」に、経歴だけが読めなければ
  その節だけが落ちる。**どちらも 500 にしない** (#519)。

  ⚠️ **h-card にも JSON-LD にも出さない。** 経歴は読み物として出すもので、機械に名乗る
  身元の一部ではない。上の「生い立ちの欄は持たない」で `dt-bday` と `birthDate` を
  持たないと決めているのと同じ線引きなので、`dt-*` / `alumniOf` / `award` を足すと
  経歴の側から崩れる。`profile-history.mf2.test.tsx` と `pages.handler.test.ts` が
  見張っている
...
### 顔はサイトのアイコン 1 つで、プロフィールでは選べない

顔は `PROFILE_PHOTO` (`app/lib/profile-fallback.ts`) が持つやんてねくんのアイコン。
出る 3 か所 (トップ・記事の末尾・`/about`) すべてで同じものを出す。

**フロントマターに顔の欄は無い。** 顔がサイトのアイコン 1 つに決まっているなら、書ける欄を
設けても選択肢は増えない。増えるのは「書き忘れて顔の無いプロフィール」という状態だけで、
それは場所によって顔が出たり出なかったりする形で表に出る。

⚠️ 名前のほうは違う。あちらはコンテンツが持ち、同期されていないときだけ
`FALLBACK_PROFILE_NAME` に倒れる。
...
作品は**コンテンツリポジトリの `works/<slug>.md` + `works/<slug>/`**。記事と同じ対の形で、
push を合図に D1 と R2 へ同期される
([ADR 0042](../../docs/adr/0042-keep-works-as-their-own-content-kind.md))。

出る場所は 3 つ。同じ 1 つのデータから賄う。

| 場所            | 出すもの                                      |
| --------------- | --------------------------------------------- |
| `/about`        | 名前・概要・在り処 (フロントマターまで)。全件 |
...
- **並びは `position`。日付では並べない。** 作った順に並べると年表になる。省略は許さず、
  同じ数を 2 つ書いたら slug で決める
- **公開範囲を持たない。** 出したくないものは push しない。`visibility` / `status` の
  欄は読まない
- **フィードを作らない。** 作ったものは購読するものではない。**それでもナビには出す**
  (ナビは「この場所には何があるか」を示すもので、購読の口があるかとは別)
- **検索の索引と埋め込みを作らない。** 探して辿り着くものではない

アセットは `/api/v1/works/<slug>/assets/<path>` で出す。コンテンツリポジトリの URL は
露出させない (記事・プロフィールのアセット API と同じ)。

⚠️ **`/about` に「すべて見る」は置かない。** 全件が出ているので、続きがあると言うことに
なる。行き先としての `/works` は節の見出し自身が持つ。

⚠️ **一覧の名乗り (題 + 一文) は `page-intro` / `page-heading` / `page-lead`。**
`/articles` と `/works` が `routes/page-intro.css` を共有する。片方だけ直すと、同じ役の
見出しが場所によって違う大きさで出る。
...
- 行き先は About / Articles / Notes / Slides / Works の 5 つ。**Notes (#412) /
  Slides (#415) はまだ中身が無く、「準備中」の一文だけを置いたページに繋がっている。**
  それでもナビには出す (サイトが何を置く場所なのかは、置き終わる前から見えていてよい)。
  中身の無いうちは `noindex` を立てて検索結果には出さない。About と Works も、
  コンテンツがまだ同期されていない間は同じ姿になる
- ⚠️ **フィードのある種別だけを並べているのではない。** Works にフィードは無い
  ([ADR 0042](../../docs/adr/0042-keep-works-as-their-own-content-kind.md))。ナビは
  「この場所には何があるか」を示すもので、購読の口があるかとは別。
  `feed-menu.test.tsx` がこの線引きを固定している
...
- ⚠️ **これは「いずれ時系列に増える種別」の話。** `/about` と `/works` は時系列に
  増えないので、購読先を実在させても何も届かない。フィードは持たない
  ([ADR 0042](../../docs/adr/0042-keep-works-as-their-own-content-kind.md))
