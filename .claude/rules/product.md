...
`/about` の名乗りは、フロントマターに `avatar` が無くても**サイトのアイコンを見える形で
出す** (`FALLBACK_PROFILE_PHOTO`)。名前だけが宙に浮くより、顔が名前の左に並んでいるほうが
「誰のページか」が一目で分かる。

⚠️ **自己紹介や出ていく先を既定で埋めないのとは扱いが違う。** あちらは書き手が書いた字と
見分けが付かなくなるが、顔のほうはサイト自身のアイコンであって誰かの言葉ではない。

**記事の末尾の筆者紹介はまだ倒していない** (`avatar` が無ければ顔を出さない)。同じ人が
場所によって顔が出たり出なかったりするので、揃えるなら一緒に直すこと。
