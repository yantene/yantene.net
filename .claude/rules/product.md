...
- 行き先は About / Articles / Notes / Slides の 4 つ。**About (#413) / Notes (#412) /
...
### 読み手に語りかける文は常体で、話し言葉を混ぜない

訳す側の文 (案内・結果・エラー) は日本語で書くが、**語調も揃える。**

```
検索できなかった。もう一度試してほしい。
リアクションは 1 つだけ。別のものを選ぶと入れ替わる。
まだ公開された記事がない。
```

- **常体。** 敬体 (です・ます) は混ぜない
- **1 文か 2 文で切る。** 3 文続けると案内ではなく喋りになる
- **話し言葉を持ち込まない。** `〜てみて` `〜の方` のような口語は既存に 1 つも無い
- 言い差し (`…もう一度。`) で終えない。動詞まで書く
- 英語側も同じ。既存は `Could not search. Please try again.` のような短い 2 文

⚠️ **メールの文面 (`services/sign-in-mail.ts`) だけは敬体。** 頼んでいない人の受信箱にも
届くものなので分けてある ([#484](https://github.com/yantene/yantene.net/issues/484))。
画面の文言をそこから写さないこと。
...
- **応答は許可の有無で変えない。** 送っても送らなくても同じ画面へ返す。分けると、
  誰が入れるのかを総当たりで数えられる
- ナビには出さない。導線はロゴの連打 ([#474](https://github.com/yantene/yantene.net/issues/474)) か、
  リンクを直接渡す経路
