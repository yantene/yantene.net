...
describe("collectBareLinkUrls", () => {
  ...
  it("文章に混ざった URL は拾わない", () => {
    const urls = collectBareLinkUrls(parse("詳しくは https://example.com/a を見よ。\n"));
    expect(urls).toEqual([]);
  });

  it("文字列がリンク先と違うリンクは拾わない", () => {
    const urls = collectBareLinkUrls(parse("[Example](https://example.com/a)\n"));
    expect(urls).toEqual([]);
  });

  it("リンクが 2 つ並んだ段落は拾わない", () => {
    const urls = collectBareLinkUrls(parse("https://example.com/a https://example.com/b\n"));
    expect(urls).toEqual([]);
  });
  ...
  it("入れ子のリスト項目の中も拾わない", () => {
    const urls = collectBareLinkUrls(parse("- 見よ\n\n  - https://example.com/a\n"));
    expect(urls).toEqual([]);
  });

  it("脚注の中は拾わない", () => {
    const urls = collectBareLinkUrls(parse("本文[^1]\n\n[^1]: https://example.com/a\n"));
    expect(urls).toEqual([]);
  });
  ...
  it("定義順を保つ", () => {
    const urls = collectBareLinkUrls(parse("https://example.com/b\n\nhttps://example.com/a\n"));
