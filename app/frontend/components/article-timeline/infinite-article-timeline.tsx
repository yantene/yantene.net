...
import { parseArticleListPayload } from "./article-list-payload";
import { ArticleTimeline } from "./article-timeline";
import type { ArticleListPayload } from "./article-list-payload";
import type { ArticleTimelineItemProps } from "./article-timeline-item";

/** 続きを取ってくる手。取れなければ投げる。 */
export type LoadArticlePage = (page: number, perPage: number) => Promise<ArticleListPayload>;

interface InfiniteArticleTimelineProps {
  /** SSR で描いた 1 ページ目。JS が動かない環境ではこれだけが残る。 */
  readonly initialArticles: readonly ArticleTimelineItemProps[];
   ...
   */
  readonly loadPage?: LoadArticlePage;
  /** 年の区切りを差し込むか (ArticleTimeline にそのまま渡す)。 */
  readonly groupByYear?: boolean;
}

/** 既定の取り方。公開 API から 1 ページぶんを読む。 */
const fetchArticlePage: LoadArticlePage = async (page, perPage) => {
 ...
 */
export function InfiniteArticleTimeline({
  initialArticles,
  totalPages,
  perPage,
  loadPage = fetchArticlePage,
  groupByYear = false,
}: InfiniteArticleTimelineProps): React.JSX.Element {
   ...
   */
  const [shownFirstPage, setShownFirstPage] = useState(initialArticles);
  if (shownFirstPage !== initialArticles) {
    setShownFirstPage(initialArticles);
  ...
  return (
    <>
      <ArticleTimeline articles={articles} groupByYear={groupByYear} />
      ...
      */}
      <p aria-live="polite" className="py-6 text-center text-sm text-base-content/60">
        ...
        {state === "idle" && !hasMore && articles.length > 0 && t("timeline.allLoaded")}
      </p>
    </>
  );
}
