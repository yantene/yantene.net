...
/**
 * 記事に付いたリアクションの数を書き換える。
 *
 * 誰が押したかは受け取らないし、保存もしない。押した本人を覚えておくのは
 * 読み手のセッション (KV) の役目で、この表には数しか残らない。
 */
export interface IArticleReactionCommandRepository {
  /** その絵文字の数を 1 増やす。行が無ければ 1 で作る。 */
  increment(articleId: string, emoji: ReactionEmoji): Promise<void>;
   ...
   */
  decrement(articleId: string, emoji: ReactionEmoji): Promise<void>;
   ...
   */
  findPublishedOn(articleId: string): Promise<string | undefined>;
   ...
   */
  addLogScore(articleId: string, weightLog: number): Promise<void>;

  /**
   * 対数スコアから重み 1 つぶんを引く。引ききったら下限に倒す。
   *
   * @param floorLogScore 下限。その記事の出発点 (投稿日の重み) を渡す。理由は
   *   domain/article-view の logScoreAfterReactionRemoved に書いてある。
   */
  subtractLogScore(articleId: string, weightLog: number, floorLogScore: number): Promise<void>;
}
