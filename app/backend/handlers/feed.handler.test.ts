...
describe("GET /feed.xml", () => {
  ...
  it("includes published articles as entries with absolute links", async () => {
    const d1 = createTestD1();
    await new D1ArticleCommandRepository(d1).upsert(
      unpersistedArticle({ slug: "hello-world", title: "Hello & World" }),
    );
...
describe("カテゴリ", () => {
  it("どの記事にも article のカテゴリが付く", async () => {
    const d1 = createTestD1();
    await new D1ArticleCommandRepository(d1).upsert(unpersistedArticle({ slug: "a", title: "A" }));
  ...
  it("?tag= を付けても無視して全体を返す", async () => {
    ...
    await command.upsert(unpersistedArticle({ slug: "a", title: "A" }));
    await command.upsert(unpersistedArticle({ slug: "b", title: "B" }));
