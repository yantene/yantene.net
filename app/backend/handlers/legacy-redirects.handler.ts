...
export const noteSlugByLegacySlug: ReadonlyMap<string, string> = new Map([
  ["i_bought_arduino", "playing-with-arduino"],
  ...
  ["first_challenge_of_topcoder", "first-topcoder-contest"],
  ["practice_of_topcoder", "practicing-topcoder"],
  ["first_challenge_of_srm", "first-topcoder-srm"],
  ["practice_practice", "practice-practice"],
  ["combsort", "comb-sort"],
  ["opencobol_in_ubuntu", "opencobol-on-ubuntu"],
  ...
  ["use_tutvpn_wisely", "use-tut-vpn-wisely"],
 ...
 * くるのは utm 等のトラッキングだけで、このサイトは外部アナリティクスを持たない
 * (CSP の connect-src は 'self' のみ)。読む先が無いものを運んでも意味がない。旧サイトが
 * 実際に使っていた /list.html の tag だけは引き継ぐ。
 */
