...
function toDimensions(width: number, height: number): ImageDimensions | undefined {
...
function readPng(view: DataView, bytes: Uint8Array): ImageDimensions | undefined {
 ...
 */
function readJpeg(view: DataView, bytes: Uint8Array): ImageDimensions | undefined {
  ...
  while (offset + 3 < bytes.length) {
    ...
    const isSof = marker >= sofMarkerMin && marker <= sofMarkerMax && !nonSofMarkers.has(marker);
    if (isSof) {
      // マーカー(2) + 長さ(2) + 精度(1) → 高さ, 幅
      if (offset + 9 > bytes.length) return undefined;
      return toDimensions(view.getUint16(offset + 7), view.getUint16(offset + 5));
    }
...
function readGif(view: DataView, bytes: Uint8Array): ImageDimensions | undefined {
 ...
 * 寸法の位置と符号化が異なる。
 */
function readWebp(view: DataView, bytes: Uint8Array): ImageDimensions | undefined {
  ...
  if (hasAsciiAt(bytes, 12, "VP8 ")) {
    ...
    return toDimensions(view.getUint16(26, true) & mask14Bit, view.getUint16(28, true) & mask14Bit);
  }
  return undefined;
}
...
export function readImageDimensions(bytes: Uint8Array): ImageDimensions | undefined {
  ...
  if (bytes.at(0) === 0xff && bytes.at(1) === 0xd8) return readJpeg(view, bytes);
