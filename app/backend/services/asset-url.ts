/**
 * 本文の中の相対的なアセット URL (画像・リンク) をアセット API URL (ルート相対) に解決する。
 *
 * 絶対 URL (http/https 等スキーム付き) やルート相対 (`/...`) は解決済みとみなしそのまま
 * 返す。相対パス (`./cover.png` / `img/a.png`) は URL 解決で `./` を畳んでから前置に繋ぐ。
 *
 * 前置が何になるかは書くもの (記事・プロフィール) ごとに違うので、呼ぶ側が渡す。
 */
...
export function resolveAssetUrlAgainst(prefix: string, url: string): string {
  const hasScheme = /^[a-z][a-z0-9+.-]*:/i.test(url);
  if (hasScheme || url.startsWith("/") || isSelfReferencing(url)) return url;

  // ダミーオリジンを与えて相対解決し、パス以降を取り出す。これで `./` は畳まれる。
  const resolved = new URL(url, `https://asset.invalid${prefix}`);
  /*
   ...
   * なる。どちらもその記事のアセットではなく、**無関係な API のパスを指す URL を
