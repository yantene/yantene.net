...
/*
 * Markdown の本文を MDAST に組む経路。
 *
 * フロントマターの中身は見ない。何が書かれていてよいかは書くもの (記事・プロフィール)
 * ごとに違うので、生の値を渡して読み手に任せる。ここが引き受けるのは「どの Markdown
 * でも同じであるべき部分」— 改行の畳み方、数式、Alert — だけ。
 */
const markdownProcessor = unified().use(remarkParse).use(remarkGfm).use(remarkMath);

export interface ParsedMarkdownBody {
  /** フロントマターから取り出した生の値 (検証前)。 */
...
/**
 * Markdown をフロントマターと本文の MDAST に分ける。
...
export function parseMarkdownBody(markdown: string): ParsedMarkdownBody {
  ...
  return { frontmatter, mdast: withMathMl(collapsed) };
}
