 ...
 * 本文の解釈 (フロントマターの除去・Alert・改行の畳み込み・数式) は
 * {@link parseMarkdownDocument} が持つ。ここが足すのは記事に固有の読み取りだけ。
 *
...
export function parseArticleContent(markdown: string): ParsedArticleContent {
  const { frontmatter, mdast } = parseMarkdownDocument(markdown);

  return {
    frontmatter: {
      title: asOptionalString(frontmatter.title),
      imageUrl: asOptionalString(frontmatter.imageUrl),
      publishedOn: asDateString(frontmatter.publishedOn),
      lastModifiedOn: asDateString(frontmatter.lastModifiedOn),
      status: asStatus(frontmatter),
    },
    mdast,
